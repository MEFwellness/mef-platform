...
## 9a. Micro-Interaction Standards — Finalized (Prompt 6)

Everything above was aspirational when written; this section records what Prompt 6 actually built and where each mechanic now lives, so a future prompt cites real code, not a plan.

**Press and lift, wired app-wide.** `.mef-press` already existed (Prompt 1) but was only applied to a fraction of the app's real buttons — Prompt 6's audit found and fixed the gap across every member-facing screen (cross-cutting chrome: `BackButton`, `AvatarLink`, `FloatingCoachLauncher`, `SignOutButton`; every auth/today/dashboard/profile/movement/insights/root-score/root-map/questionnaires/food-lens/protein-ledger/pantry/restaurant/exercise-library/welcome/onboarding/conversation button that lacked it). `.mef-card-lift` (Bible §9's own "not yet a named class" note) is now real, both as the raw CSS class and as `<Card lift>` (`components/layout/Card.tsx`) — wired onto every genuinely tappable card found in the audit (profile's four settings cards, movement's two hub cards, assessment catalog "last result" cards, `CategoryCard`, the food-lens secondary card row, Today's hero check-in card), while every pure-display card was left untouched.

**Selection ripple, unified beyond check-in.** `TapBleedTile` (`components/checkin/scales/shared.tsx`) was already the one sanctioned language for check-in — Prompt 6 extended it everywhere else a member makes a choice, via a new reusable hook, `lib/motion/useBleedTap.ts`, that factors out just the tap-origin-capture half so a control with its own bespoke shape (not TapBleedTile's fixed pill) can still bleed-fill from the real tap point and fire one `triggerHaptic()`. Retrofitted this way: the generic points-scored engine and its context-gate questions (now sharing one `components/assessments/QuestionOptionButton.tsx`), WBSA's boolean/frequency/single/multi-select rows, Core Values Snapshot's single-select rows/1-5 scale pills/Q12 forced-choice tiles (each keeping its own existing fill color — forest green or `CVS_GOLD` — the ripple only changes how that color arrives), Readiness Pulse and Life Signal Check (inherit CVS's `SingleSelectQuestion` directly), onboarding's enum/multi-select tiles and concern-confirm control, Welcome's goal-selection and primary-goal tiles, and the guest wellness-check preview's option tiles (layered on top of that screen's own pre-existing `active:scale`/`hover:scale` transforms, not replacing them).

**Deliberately kept as their own bespoke language, not folded into the ripple standard:** the mood-face and moon hero visualizations in `components/checkin/scales/FiveFacesScale.tsx`/`FiveMoonsScale.tsx` (their gold-ring-on-selection treatment and mood-face spring predate this prompt and are explicitly preserved, per this Bible's own original §9 note); Primal Pattern's checkbox-card (`components/primal-pattern/PrimalPatternQuestionCard.tsx`) keeps its own light-tint fill + animated badge + `active:scale`, since a saturated ripple bleed would visually clash with a fill this subtle — it gained the missing haptic tick, nothing else. Both are the same kind of considered exception the mood-face spring already was, not an oversight.

**Haptics.** Confirmed and extended the existing rule (`lib/haptics.ts`, Vibration API, silent no-op on iOS Safari/desktop/anywhere unsupported — this is expected, not a bug to work around): every selection tap fires one haptic tick (already true for check-in, now true everywhere via the retrofits above, plus movement/equipment toggle pills), and `SuccessCheck` (below) fires one on mount as the completion haptic for a routine save. Nothing fires per-scroll, per-keystroke, or per-render.

**Success states — the missing piece, now built.** `components/motion/SuccessCheck.tsx` is a small ring-plus-checkmark that draws itself once per mount, reusing `.mef-close-check-circle`/`.mef-close-check-draw` (the first real consumer of `.mef-close-check-circle`, which existed in CSS with no caller before this prompt) — deliberately never the gold sweep, per this Bible's own §9 rule. Wired into: WBSA's and the generic engine's completion cards (replacing a static Lucide `CheckCircle2`), a protein target save, a logged/saved meal in Food Lens, and a logged Protein Ledger entry. Primal Pattern's completion still has no intermediate screen at all (`router.push`s straight to results) — deliberately left alone, since adding one would be a new screen/delay this prompt's own hard boundary forbids; it did get its in-flight button label upgraded to `ROOT_FINISHING_LABEL` instead (see Loading below).

**Progress feedback.** `components/motion/GrowBar.tsx` and a newly-generalized `components/motion/CountUp.tsx` (the same raf/cubic-ease/reduced-motion-aware logic `RootScoreCountUp` already had, Prompt 5 — that file now just re-exports `CountUp` under its own name, so the dashboard hero is untouched) existed with zero real consumers before this prompt. Now wired into the Protein Ledger's daily tally (bar + count-up gram number) and its 7-day history bars, the two in-session-updating progress displays the audit found.

**Loading, the remaining gaps.** Three assessment takers (generic engine, WBSA, Primal Pattern) had a bare `Loader2` glued to a static button label while completing, unlike Core Values Snapshot/Readiness Pulse/Life Signal Check's own dedicated `ROOT_FINISHING_LABEL` finishing beat (`lib/reveal/copy.ts`) — generic engine and WBSA now render that same full-screen finishing state (a presentational change only: the wait itself, and everything that happens during it, is unchanged); Primal Pattern's button label swaps to the same copy instead, since it has no separate finishing screen to render into. Two inline component-level spinners also had bare/under-described text with no icon or vice versa: `FloatingCoachPanel.tsx`'s "Loading your conversation…" gained a spinner icon to match every other loading treatment in the app, and `ExerciseDetailView.tsx`'s bare "Loading…" over a video poster became "Cueing up your video…".

**Empty states, the remaining gaps.** Reworded to Root's first-person voice and, where the state is a whole screen's only content, given one small living element (a gently breathing icon via `components/motion/Breathe.tsx`, never more than one per screen per §10): `/notifications` (empty inbox), `/today`'s not-yet-ready coaching lesson, the Protein Ledger's empty entry list, `/progress`'s empty check-in history and "not enough data" line, and `/insights`' no-patterns-yet state (kept out of `<CenterStage>` since real content already sits above it on that screen — wrapping it would create dead space, the exact case §6a's own centering rule warns against).
...
- **Prompt 6: Micro-Interactions.** *(Complete.)* Button/card press-and-lift feedback wired app-wide, selection ripple unified beyond check-in, haptics extended, success states and progress feedback built and wired, remaining loading/empty-state gaps closed — see §9a.
...
*End of v1.4. This document should be updated, not superseded, as Prompts 1–8 land — each prompt's actual implementation should be checked against its row above and this Bible corrected if reality diverges from the plan.*
