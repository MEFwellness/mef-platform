...
type OpenLicenseImageRow = { id: string; external_id: string; storage_path: string };
type ExtractedPosterRow = { id: string; storage_path: string };
...
async function main() {
  ...
  const { rows: images, error: imagesError } = await selectAllRows<OpenLicenseImageRow, PostgrestError | null>(() =>
    supabase.from('exercise_open_license_images').select('*').order('id', { ascending: true })
  );
  ...
  if (imageRows.length > 0) {
    ...
    const { error: deleteError } = await writeInChunks(
      imageRows.map((r) => r.id),
      (chunk) => supabase.from('exercise_open_license_images').delete().in('id', chunk)
    );
  ...
  const { rows: posters, error: postersError } = await selectAllRows<ExtractedPosterRow, PostgrestError | null>(() =>
    supabase
  ...
  if (posterRows.length > 0) {
    ...
    const { error: deleteError } = await writeInChunks(
      ...
      (chunk) => supabase.from('exercise_extracted_posters').delete().in('id', chunk)
    );
