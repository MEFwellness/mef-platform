...
/**
 * The dashboard card for The Weight of Yes's seven day experiment.
 *
 ...
 * swallowed one, which is the thing the seven days are for noticing, so
...
import {
  logTheWeightOfYesDayAction,
  type TwoyExperimentStatus,
} from '@/app/actions/theWeightOfYes';

export function TheWeightOfYesExperimentPanel({ status }: { status: TwoyExperimentStatus }) {
  ...
  function logDay(completed: boolean) {
    ...
    startTransition(async () => {
      const result = await logTheWeightOfYesDayAction(status.experiment.id, completed);
      if (!result.ok) {
    ...
    <div className={`${CVS_CARD} mef-animate-in p-7`}>
      ...
      {todayCompleted !== null ? (
        <div className="mt-5 flex items-center gap-2 text-sm font-medium text-[#4F7A63]">
          ...
          {todayCompleted ? 'Logged: you said one.' : 'Logged: you noticed one you swallowed.'}
        </div>
      ) : (
