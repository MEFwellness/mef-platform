...
export function WhereYourJoyLivesExperience({
  ...
  const landingStep = written === 0 ? INTRO_STEP : Math.min(written, WYJL_QUESTIONS.length - 1);
  ...
  const motion = useHappinessSittingMotion(
    initialSeenKeys(
      WYJL_QUESTIONS.filter((entry) => isAnswered(initialDraft[entry.key])).map(
        (entry) => entry.key
      ),
      landingStep >= FIRST_QUESTION_STEP ? (WYJL_QUESTIONS[landingStep]?.key ?? null) : null
    )
  );
  ...
  function advance() {
    ...
    if (!isLastQuestion) {
      startTransition(async () => {
        ...
        const next = WYJL_QUESTIONS[step + 1];
      ...
      <div className={PANEL}>
        ...
        {completed && (
          <div className="relative mt-6">
            <JoyPair answers={completed.answers} instant />
          </div>
        )}
  ...
  return (
    <div className={PANEL}>
      ...
      {question && (
        <QuestionStage
          counter={`Question ${step + 1} of ${WYJL_QUESTIONS.length}`}
      ...
      */}
        ...
        <div className="relative mt-6">
          ...
          <ClosingTail
            delayMs={hddClosingTailDelayMs(
              WYJL_CLOSING_PAIR_KEYS.reduce(
                (total, key) => total + hddClosingLines(finished.answers[key] ?? '').length,
                0
              ),
              true
            )}
      ...
      {step === DONE_STEP && (
        <div className="relative mt-4">
          {finished && <JoyPair answers={finished.answers} instant />}
          {experimentNote && (
 ...
 * writing away.
 */
function JoyPair({ answers, instant = false }: { answers: WyjlAnswers; instant?: boolean }) {
  return (
    <ClosingCenterpiece
      layout="pair"
      entries={WYJL_CLOSING_PAIR_KEYS.map((key) => ({
        caption: questionFor(key)?.prompt ?? '',
        captionTone: 'prompt' as const,
        text: answers[key] ?? '',
      }))}
      fixedLine={WYJL_CLOSING_LINE}
      instant={instant}
    />
  );
}
