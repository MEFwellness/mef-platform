...
/**
 * The dashboard card for Where Your Joy Lives's seven day experiment.
 *
 ...
 * Readiness Pulse and Owning Your Value experiments all log into. No new
...
import {
  logWhereYourJoyLivesDayAction,
  type WyjlExperimentStatus,
} from '@/app/actions/whereYourJoyLives';

export function WhereYourJoyLivesExperimentPanel({ status }: { status: WyjlExperimentStatus }) {
  ...
  function logDay(completed: boolean) {
    ...
    startTransition(async () => {
      const result = await logWhereYourJoyLivesDayAction(status.experiment.id, completed);
      if (!result.ok) {
