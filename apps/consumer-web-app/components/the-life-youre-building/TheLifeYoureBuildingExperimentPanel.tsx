...
/**
 * The dashboard card for The Life You're Building's seven day experiment.
 *
 ...
 *
 * BOTH ANSWERS ARE REAL ANSWERS HERE. "Not today" means the day held
 * nothing belonging to the life she is building, which is worth noticing
 * rather than worth hiding, so neither button is the failure button and the
 * logged line says so.
 *
...
import {
  logTheLifeYoureBuildingDayAction,
  type TlybExperimentStatus,
} from '@/app/actions/theLifeYoureBuilding';

export function TheLifeYoureBuildingExperimentPanel({
  status,
}: {
  status: TlybExperimentStatus;
}) {
  ...
  function logDay(completed: boolean) {
    ...
    startTransition(async () => {
      const result = await logTheLifeYoureBuildingDayAction(status.experiment.id, completed);
      if (!result.ok) {
    ...
    <div className={`${CVS_CARD} mef-animate-in p-7`}>
      ...
      {todayCompleted !== null ? (
        <div className="mt-5 flex items-center gap-2 text-sm font-medium text-[#4F7A63]">
          ...
          {todayCompleted ? 'Logged: yes, something did.' : 'Logged: not today.'}
        </div>
      ) : (
