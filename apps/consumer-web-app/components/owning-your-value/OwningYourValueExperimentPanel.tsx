...
/**
 * The dashboard card for Owning Your Value's seven day experiment.
 *
 * THE QUESTION IS THE ACTION, in the same words she agreed to. A member on
 * day 4 should not have to remember what she signed up for, and a card that
 ...
 * the same table the Core Values Snapshot, the Life Signal Check and the
 * Readiness Pulse experiments all log into. No new table and no new column.
 *
...
import {
  logOwningYourValueDayAction,
  type OyvExperimentStatus,
} from '@/app/actions/owningYourValue';

export function OwningYourValueExperimentPanel({ status }: { status: OyvExperimentStatus }) {
  ...
  function logDay(completed: boolean) {
    ...
    startTransition(async () => {
      const result = await logOwningYourValueDayAction(status.experiment.id, completed);
      if (!result.ok) {
