...
export function RootMessagePopupClient({ message }: { message: RootPopupMessage }) {
  ...
  if (message.kind === 'the_weight_of_yes_assigned') {
    const m: TheWeightOfYesMessage = message;
    return (
      <>
        {/* The same receipt as the five branches above, on the same table:
            this is an assessment_assignments row like any other coach
            assignment, so it gets the one receipt system rather than a
            second one of its own. */}
        <TrackAssignmentDelivered assignmentId={m.assignmentId} presentation="popup" />
        <RootInvitePopup
          eyebrow={TWOY_COPY.popupEyebrow}
          title={m.title}
          body={m.body}
          ctaLabel={TWOY_COPY.popupCta}
          href={m.primaryHref}
          isPending={isPending}
          onMaybeLater={handleMaybeLater}
          onIgnore={handleIgnore}
  ...
  if (message.kind === 'your_own_company_assigned') {
    const m: YourOwnCompanyMessage = message;
    return (
      <>
        {/* The same receipt as the eight branches above, on the same table:
            this is an assessment_assignments row like any other coach
            assignment, so it gets the one receipt system rather than a
            second one of its own. */}
        <TrackAssignmentDelivered assignmentId={m.assignmentId} presentation="popup" />
        <RootInvitePopup
          eyebrow={YOC_COPY.popupEyebrow}
          title={m.title}
          body={m.body}
          ctaLabel={YOC_COPY.popupCta}
          href={m.primaryHref}
          isPending={isPending}
          onMaybeLater={handleMaybeLater}
          onIgnore={handleIgnore}
