...
/**
 * The dashboard card for Being Seen's seven day experiment.
 *
 ...
 *
 * BOTH ANSWERS ARE REAL ANSWERS HERE. "Not today" on this card means she
 * kept something in, which is the thing the seven days are for noticing, so
 * neither button is the failure button and the logged line says so.
 *
...
import { logBeingSeenDayAction, type BsnExperimentStatus } from '@/app/actions/beingSeen';

export function BeingSeenExperimentPanel({ status }: { status: BsnExperimentStatus }) {
  ...
  function logDay(completed: boolean) {
    ...
    startTransition(async () => {
      const result = await logBeingSeenDayAction(status.experiment.id, completed);
      if (!result.ok) {
    ...
    <div className={`${CVS_CARD} mef-animate-in p-7`}>
      ...
      {todayCompleted !== null ? (
        <div className="mt-5 flex items-center gap-2 text-sm font-medium text-[#4F7A63]">
          ...
          {todayCompleted ? 'Logged: you showed something.' : 'Logged: you noticed one you kept in.'}
        </div>
      ) : (
