...
/**
 * The dashboard card for Your Own Company's seven day experiment.
 *
 ...
 *
 * BOTH ANSWERS ARE REAL ANSWERS HERE. "Not today" means she did not catch
...
import {
  logYourOwnCompanyDayAction,
  type YocExperimentStatus,
} from '@/app/actions/yourOwnCompany';

export function YourOwnCompanyExperimentPanel({ status }: { status: YocExperimentStatus }) {
  ...
  function logDay(completed: boolean) {
    ...
    startTransition(async () => {
      const result = await logYourOwnCompanyDayAction(status.experiment.id, completed);
      if (!result.ok) {
    ...
    <div className={`${CVS_CARD} mef-animate-in p-7`}>
      ...
      {todayCompleted !== null ? (
        <div className="mt-5 flex items-center gap-2 text-sm font-medium text-[#4F7A63]">
          ...
          {todayCompleted ? 'Logged: you caught it.' : 'Logged: not today.'}
        </div>
      ) : (
