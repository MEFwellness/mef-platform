...
export function buildStillBuildingSentence(checkinCount: number, daysSinceFirstCheckin: number | null): string {
  const encouragement =
    "I don't usually have a real finding this early, that's expected, not a problem.";

  if (checkinCount === 0) {
    return "You haven't logged a check-in yet. I'll start filling this in once you have.";
  }
