 ...
 * account may read or write, and migration 225's insert policy requires a
...
import type { WbsAssignment, WbsSessionRecord } from './data';

export type WbsAccess =
  | { kind: 'assigned'; assignment: WbsAssignment }
  | { kind: 'completed'; session: WbsSessionRecord }
  | { kind: 'none' };

export function resolveWbsAccess(input: {
  assignmentRead: { ok: boolean; assignment: WbsAssignment | null };
  sessionRead: { ok: boolean; records: WbsSessionRecord[] };
}): WbsAccess {
