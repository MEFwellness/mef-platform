/**
 * Who The Life You're Building is for. One rule, and every surface in the
 ...
 *
 * IT IS NOT GATED ON ANY TEMPLATE BEFORE IT, INCLUDING THE ONE IT CAN
 * FOLLOW. A coach may start any member here, including one who has sat with
 * none of the seven templates beside it and including one who has never
 * done Owning Your Value. This file has never heard of any of them, does
 * not import one, and cannot be made to care. Whether a sitting runs as a
 * follow-up is decided in ./followUp.ts, after this gate has already said
 * yes, and it can never decide whether she gets in. Requiring an earlier
 * sitting would be exactly the second invisible lock the standing rules
 * forbid.
 *
 * FAILS SHUT, the same direction the seven templates beside it fail. A
 * failed read of her assignments resolves to "not offered" rather than to
 ...
 * any account may read or write, and migration 219's insert policy requires
...
import type { TlybAssignment, TlybSessionRecord } from './data';

export type TlybAccess =
  /** She has an open assignment and has not finished it. */
  | { kind: 'assigned'; assignment: TlybAssignment }
  /** No open assignment, but she has finished at least one sitting before. */
  | { kind: 'completed'; session: TlybSessionRecord }
 ...
 */
export function resolveTlybAccess(input: {
  assignmentRead: { ok: boolean; assignment: TlybAssignment | null };
  sessionRead: { ok: boolean; records: TlybSessionRecord[] };
}): TlybAccess {
