 ...
 * member's Where Your Joy Lives state right now.
 *
...
import { resolveWyjlAccess } from './access';
import {
  fetchWyjlSessionForAssignment,
  fetchPendingWyjlAssignment,
  listWyjlSessions,
  type WyjlSessionRecord,
} from './data';
import { WYJL_QUESTIONS_VERSION, type WyjlDraft } from './questions';

export type WyjlState =
  | {
      status: 'pending';
      assignmentId: string;
      assignedAt: string;
  ...
  | {
      status: 'completed';
      session: WyjlSessionRecord;
    };

export async function buildWyjlState(
  supabase: SupabaseClient,
  memberId: string
): Promise<WyjlState | null> {
  const assignmentRead = await fetchPendingWyjlAssignment(supabase, memberId);
  ...
  const sessionRead =
    assignmentRead.ok && !assignmentRead.assignment
      ? await listWyjlSessions(supabase, memberId, 1)
      : { ok: true, records: [] };

  // ONE DECISION, MADE IN ONE PLACE. This function does not re-check
  // anything resolveWyjlAccess already decides; it only turns that decision
  // into the shape the surfaces render.
  const access = resolveWyjlAccess({ assignmentRead, sessionRead });
  switch (access.kind) {
    case 'assigned': {
      ...
      const draftRow = await fetchWyjlSessionForAssignment(
        supabase,
        memberId,
