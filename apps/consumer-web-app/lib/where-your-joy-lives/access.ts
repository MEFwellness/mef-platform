/**
 * Who Where Your Joy Lives is for. One rule, and every surface in the
 ...
 *
 * IT IS ALSO NOT GATED ON OWNING YOUR VALUE. This is the second Happiness
 * template and its copy greets a member who has sat with Root before, but
 * "sat with Root before" is a thing the coach knows and this app does not
 * check. Requiring a prior sitting would be exactly the second invisible
 * lock the standing rules forbid, and it would take this experience away
 * from a member her coach deliberately started here.
 *
 * FAILS SHUT, the same direction lib/owning-your-value/access.ts fails. A
 ...
 * any account may read or write, and migration 212's insert policy requires
...
import type { WyjlAssignment, WyjlSessionRecord } from './data';

export type WyjlAccess =
  /** She has an open assignment and has not finished it. */
  | { kind: 'assigned'; assignment: WyjlAssignment }
  /** No open assignment, but she has finished at least one sitting before. */
  | { kind: 'completed'; session: WyjlSessionRecord }
 ...
 */
export function resolveWyjlAccess(input: {
  assignmentRead: { ok: boolean; assignment: WyjlAssignment | null };
  sessionRead: { ok: boolean; records: WyjlSessionRecord[] };
}): WyjlAccess {
