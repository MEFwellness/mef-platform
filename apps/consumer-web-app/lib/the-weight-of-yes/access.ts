/**
 * Who The Weight of Yes is for. One rule, and every surface in the feature
 ...
 *
 * IT IS NOT GATED ON ANY TEMPLATE BEFORE IT, AND THAT MATTERS MORE HERE
 * THAN ANYWHERE. This is the one template that can FOLLOW an earlier one,
 * and a follow-up is the easiest place in this codebase to accidentally
 * build a prerequisite. There is none. This file has never heard of the
 * earlier template, does not import it, and cannot be made to care whether
 * she finished it: the follow-up is a question's WORDING, decided in
 * ./followUp.ts, and it can never decide whether she gets in.
 *
 * FAILS SHUT, the same direction the three templates beside it fail. A
 ...
 * any account may read or write, and migration 214's insert policy requires
...
import type { TwoyAssignment, TwoySessionRecord } from './data';

export type TwoyAccess =
  /** She has an open assignment and has not finished it. */
  | { kind: 'assigned'; assignment: TwoyAssignment }
  /** No open assignment, but she has finished at least one sitting before. */
  | { kind: 'completed'; session: TwoySessionRecord }
 ...
 */
export function resolveTwoyAccess(input: {
  assignmentRead: { ok: boolean; assignment: TwoyAssignment | null };
  sessionRead: { ok: boolean; records: TwoySessionRecord[] };
}): TwoyAccess {
