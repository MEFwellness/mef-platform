/**
 * The Weight of Yes on the shared Happiness table (migrations 211, 212, 213
 * and 214), and the reads its gate needs.
 *
 ...
 *
 * EVERY READ IS SCOPED BY experience_key. The table now holds FOUR
 ...
 *
 * THE FOLLOW-UP FLAG IS WRITTEN ONCE, ON THE INSERT, and never touched
 * again. It records which version of question one actually ran, so it must
 * not drift with anything the member does afterwards. See
 * ./followUp.ts for why the check happens when it happens.
 */

import type { SupabaseClient } from '@supabase/supabase-js';
import { HAPPINESS_DEEP_DIVE_TABLE, TWOY_DEFINITION_ID, TWOY_KEY } from './constants';
import { readTwoyAnswers, sanitizeTwoyDraft, type TwoyAnswers, type TwoyDraft } from './questions';

const SESSION_COLUMNS =
  'id, assignment_id, experience_key, questions_version, answers, kind_no, follow_up_source_experience_key, started_at, completed_at, created_at';

export type TwoySessionRecord = {
  id: string;
   ...
   * version of question one actually ran. Null means the standalone version
   * ran, and that is the only reading anyone may take from a null.
   */
...
function fromRow(row: SessionRow): TwoySessionRecord {
  return {
    id: row.id,
    assignmentId: row.assignment_id,
    questionsVersion: row.questions_version,
    draft: sanitizeTwoyDraft(row.answers) ?? {},
    answers: readTwoyAnswers(row.answers),
    kindNo: row.kind_no,
 ...
 *
 * "No row" and "the read did not work" are kept apart, exactly as the three
 ...
 */
export async function fetchPendingTwoyAssignment(
  supabase: SupabaseClient,
  memberId: string
): Promise<{ ok: boolean; assignment: TwoyAssignment | null }> {
  const { data, error } = await supabase
    .from('assessment_assignments')
    .select('id, created_at, reason, due_at')
    .eq('member_id', memberId)
    .eq('assessment_definition_id', TWOY_DEFINITION_ID)
    .eq('status', 'pending')
  ...
  if (error) {
    console.error('fetchPendingTwoyAssignment failed', error);
 ...
 */
export async function listTwoySessions(
  supabase: SupabaseClient,
  memberId: string,
  limit = 24
): Promise<{ ok: boolean; records: TwoySessionRecord[] }> {
  const { data, error } = await supabase
    .from(HAPPINESS_DEEP_DIVE_TABLE)
    .select(SESSION_COLUMNS)
    .eq('member_id', memberId)
    .eq('experience_key', TWOY_KEY)
    .not('completed_at', 'is', null)
    .order('completed_at', { ascending: false })
    .limit(limit);

  if (error) {
    console.error('listTwoySessions failed', error);
 ...
 * return one of the three other templates' sittings that share this table.
 */
export async function fetchTwoySessionForAssignment(
  supabase: SupabaseClient,
  memberId: string,
  assignmentId: string
): Promise<TwoySessionRecord | null> {
  const { data, error } = await supabase
    .from(HAPPINESS_DEEP_DIVE_TABLE)
    .select(SESSION_COLUMNS)
    .eq('member_id', memberId)
  ...
  if (error) {
    console.error('fetchTwoySessionForAssignment failed', error);
    return null;
  }
 ...
 * already records which version of question one she was shown, and
 * overwriting that later would let a completion of the earlier template
 * halfway through this sitting rewrite the history of a question she has
 * already answered.
 *
 ...
 */
export async function saveTwoyDraft(
  supabase: SupabaseClient,
  memberId: string,
  params: {
    assignmentId: string;
  ...
  const existing = await fetchTwoySessionForAssignment(supabase, memberId, params.assignmentId);
  ...
  if (existing) {
    const { data, error } = await supabase
      ...
      .select(SESSION_COLUMNS)
      .maybeSingle();
    if (error) console.error('saveTwoyDraft update failed', error);
    return data ? fromRow(data as unknown as SessionRow) : null;
  }
  ...
  if (error) console.error('saveTwoyDraft insert failed', error);
  return await fetchTwoySessionForAssignment(supabase, memberId, params.assignmentId);
}

/**
 * Stamps her sitting finished, with the complete sheet and the kind version
 * of her no in its own column.
 *
 ...
 *
 * The follow-up flag is NOT part of this update. It was decided and written
 * when the row was created, and a completion is not the moment to revisit
 * which question she was actually asked.
 */
export async function completeTwoySession(
  supabase: SupabaseClient,
  memberId: string,
  params: {
    assignmentId: string;
  ...
  const existing =
    (await fetchTwoySessionForAssignment(supabase, memberId, params.assignmentId)) ??
    (await saveTwoyDraft(supabase, memberId, {
  ...
  if (error) console.error('completeTwoySession failed', error);
