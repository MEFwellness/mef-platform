/**
 * Who Owning Your Value is for. One rule, and every surface in the feature
 ...
 *
 * FAILS SHUT, the same direction lib/stress-load/access.ts fails. A failed
 ...
 * any account may read or write, and migration 211's insert policy requires
 ...
 * before it renders, and every server action re-asks before it writes.
 */

import type { OyvAssignment, OyvSessionRecord } from './data';

export type OyvAccess =
  /** She has an open assignment and has not finished it. */
  | { kind: 'assigned'; assignment: OyvAssignment }
  /** No open assignment, but she has finished at least one sitting before. */
  | { kind: 'completed'; session: OyvSessionRecord }
 ...
 */
export function resolveOyvAccess(input: {
  assignmentRead: { ok: boolean; assignment: OyvAssignment | null };
  sessionRead: { ok: boolean; records: OyvSessionRecord[] };
}): OyvAccess {
