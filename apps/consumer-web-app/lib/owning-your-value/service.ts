 ...
 * member's Owning Your Value state right now.
 *
 ...
 *              submit will answer, and whatever she has already written, so
 *              the route can put her back where she stopped.
 *   completed  finished. Carries her nine answers and the held sentence.
 *
...
import { resolveOyvAccess } from './access';
import {
  fetchOyvSessionForAssignment,
  fetchPendingOyvAssignment,
  listOyvSessions,
  type OyvSessionRecord,
} from './data';
import { OYV_QUESTIONS_VERSION, type OyvDraft } from './questions';

export type OyvState =
  | {
      status: 'pending';
      assignmentId: string;
      assignedAt: string;
  ...
  | {
      status: 'completed';
      session: OyvSessionRecord;
    };

export async function buildOyvState(
  supabase: SupabaseClient,
  memberId: string
): Promise<OyvState | null> {
  const assignmentRead = await fetchPendingOyvAssignment(supabase, memberId);
  ...
  const sessionRead =
    assignmentRead.ok && !assignmentRead.assignment
      ? await listOyvSessions(supabase, memberId, 1)
      : { ok: true, records: [] };

  // ONE DECISION, MADE IN ONE PLACE. This function does not re-check
  // anything resolveOyvAccess already decides; it only turns that decision
  // into the shape the surfaces render.
  const access = resolveOyvAccess({ assignmentRead, sessionRead });
  switch (access.kind) {
    case 'assigned': {
      ...
      const draftRow = await fetchOyvSessionForAssignment(
        supabase,
        memberId,
