/**
 * Who Being Seen is for. One rule, and every surface in the feature is on
 * one side of it.
 *
 ...
 * here, including one who has sat with none of the four templates beside
 ...
 *
 * FAILS SHUT, the same direction the four templates beside it fail. A
 ...
 * any account may read or write, and migration 215's insert policy requires
...
import type { BsnAssignment, BsnSessionRecord } from './data';

export type BsnAccess =
  /** She has an open assignment and has not finished it. */
  | { kind: 'assigned'; assignment: BsnAssignment }
  /** No open assignment, but she has finished at least one sitting before. */
  | { kind: 'completed'; session: BsnSessionRecord }
 ...
 */
export function resolveBsnAccess(input: {
  assignmentRead: { ok: boolean; assignment: BsnAssignment | null };
  sessionRead: { ok: boolean; records: BsnSessionRecord[] };
}): BsnAccess {
