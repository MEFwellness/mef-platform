/**
 * Your Own Company's experiment, on the existing daily log table.
 *
 ...
 * day. Life Signal Check, the Readiness Pulse and all six Happiness
...
import { YOC_EXPERIENCE_KEY } from './constants';

export {
  listCvsDailyLogs as listYocDailyLogs,
  upsertCvsDailyLog as upsertYocDailyLog,
} from '../core-values-snapshot/dailyLogsData';

/** Her most recent Your Own Company experiment, whatever its status. */
export async function findLatestYocExperiment(
  ...
  const { data, error } = await supabase
    ...
    .eq('source_experience_key', YOC_EXPERIENCE_KEY)
