 ...
 * member's Breathing Pattern Check-In state right now.
 *
...
import { BPC_CONTENT_VERSION } from './constants';
import { resolveBpcAccess } from './access';
import {
  fetchBpcSessionForAssignment,
  fetchPendingBpcAssignment,
  listBpcSessions,
  type BpcSessionRecord,
} from './data';

export type BpcState =
  ...
  | {
      ...
      session: BpcSessionRecord;
    }
  | { status: 'completed'; session: BpcSessionRecord };

export async function buildBpcState(
  supabase: SupabaseClient,
  memberId: string
): Promise<BpcState | null> {
  const assignmentRead = await fetchPendingBpcAssignment(supabase, memberId);
  ...
  const sessionRead =
    assignmentRead.ok && !assignmentRead.assignment
      ? await listBpcSessions(supabase, memberId, 1)
      : { ok: true, records: [] };

  const access = resolveBpcAccess({ assignmentRead, sessionRead });

  switch (access.kind) {
    case 'assigned': {
      const open = await fetchBpcSessionForAssignment(supabase, memberId, access.assignment.id);
