/**
 * Who The Giving Ledger is for. One rule, and every surface in the feature
 ...
 *
 * IT IS NOT GATED ON EITHER TEMPLATE BEFORE IT. This is the third Happiness
 * template and a coach may start any member here. Requiring a prior sitting
 * would be exactly the second invisible lock the standing rules forbid, and
 * it would take this experience away from a member her coach deliberately
 * began with.
 *
 * FAILS SHUT, the same direction the two templates beside it fail. A failed
 ...
 * any account may read or write, and migration 213's insert policy requires
...
import type { TglAssignment, TglSessionRecord } from './data';

export type TglAccess =
  /** She has an open assignment and has not finished it. */
  | { kind: 'assigned'; assignment: TglAssignment }
  /** No open assignment, but she has finished at least one sitting before. */
  | { kind: 'completed'; session: TglSessionRecord }
 ...
 */
export function resolveTglAccess(input: {
  assignmentRead: { ok: boolean; assignment: TglAssignment | null };
  sessionRead: { ok: boolean; records: TglSessionRecord[] };
}): TglAccess {
