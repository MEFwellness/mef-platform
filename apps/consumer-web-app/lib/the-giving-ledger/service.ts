 ...
 * member's The Giving Ledger state right now.
 *
...
import { resolveTglAccess } from './access';
import {
  fetchTglSessionForAssignment,
  fetchPendingTglAssignment,
  listTglSessions,
  type TglSessionRecord,
} from './data';
import { TGL_QUESTIONS_VERSION, type TglDraft } from './questions';

export type TglState =
  | {
      status: 'pending';
      assignmentId: string;
      assignedAt: string;
  ...
  | {
      status: 'completed';
      session: TglSessionRecord;
    };

export async function buildTglState(
  supabase: SupabaseClient,
  memberId: string
): Promise<TglState | null> {
  const assignmentRead = await fetchPendingTglAssignment(supabase, memberId);
  ...
  const sessionRead =
    assignmentRead.ok && !assignmentRead.assignment
      ? await listTglSessions(supabase, memberId, 1)
      : { ok: true, records: [] };

  // ONE DECISION, MADE IN ONE PLACE. This function does not re-check
  // anything resolveTglAccess already decides; it only turns that decision
  // into the shape the surfaces render.
  const access = resolveTglAccess({ assignmentRead, sessionRead });
  switch (access.kind) {
    case 'assigned': {
      ...
      const draftRow = await fetchTglSessionForAssignment(
        supabase,
        memberId,
