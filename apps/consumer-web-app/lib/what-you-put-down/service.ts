 ...
 * member's What You Put Down state right now.
 *
 ...
 *              whatever is already on her shelf.
 *   completed  finished. Carries her answers and her shelf.
 *
...
import { resolveWypdAccess } from './access';
import {
  fetchWypdSessionForAssignment,
  fetchPendingWypdAssignment,
  listWypdSessions,
  type WypdSessionRecord,
} from './data';
import { WYPD_QUESTIONS_VERSION, type WypdDraft } from './questions';
import { WYPD_EMPTY_SHELF, type WypdShelfState } from './shelf';

export type WypdState =
  | {
      status: 'pending';
      assignmentId: string;
      assignedAt: string;
      ...
      draft: WypdDraft;
      /** Her shelf so far, empty when she has not started. */
      shelf: WypdShelfState;
    }
  | {
      status: 'completed';
      session: WypdSessionRecord;
    };

export async function buildWypdState(
  supabase: SupabaseClient,
  memberId: string
): Promise<WypdState | null> {
  const assignmentRead = await fetchPendingWypdAssignment(supabase, memberId);
  ...
  const sessionRead =
    assignmentRead.ok && !assignmentRead.assignment
      ? await listWypdSessions(supabase, memberId, 1)
      : { ok: true, records: [] };

  // ONE DECISION, MADE IN ONE PLACE. This function does not re-check
  // anything resolveWypdAccess already decides; it only turns that decision
  // into the shape the surfaces render.
  const access = resolveWypdAccess({ assignmentRead, sessionRead });
  switch (access.kind) {
    case 'assigned': {
      ...
      const draftRow = await fetchWypdSessionForAssignment(
        supabase,
        memberId,
      ...
      return {
        ...
        shelf: resumable?.shelf ?? WYPD_EMPTY_SHELF,
      };
    }
    case 'completed':
