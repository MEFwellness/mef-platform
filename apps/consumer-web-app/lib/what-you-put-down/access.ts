/**
 * Who What You Put Down is for. One rule, and every surface in the feature
 ...
 * here, including one who has sat with none of the five templates beside
 ...
 *
 * FAILS SHUT, the same direction the five templates beside it fail. A
 ...
 * any account may read or write, and migration 217's insert policy requires
...
import type { WypdAssignment, WypdSessionRecord } from './data';

export type WypdAccess =
  /** She has an open assignment and has not finished it. */
  | { kind: 'assigned'; assignment: WypdAssignment }
  /** No open assignment, but she has finished at least one sitting before. */
  | { kind: 'completed'; session: WypdSessionRecord }
 ...
 */
export function resolveWypdAccess(input: {
  assignmentRead: { ok: boolean; assignment: WypdAssignment | null };
  sessionRead: { ok: boolean; records: WypdSessionRecord[] };
}): WypdAccess {
