...
import { beginLscAction, retakeLscAction } from '@/app/actions/lifeSignalCheck';
...
export default async function LifeSignalCheckOverviewPage() {
  ...
  const [isCoach, access] = await Promise.all([
    ...
    checkAssessmentAccess(supabase, user.id, LSC_KEY, { intent: 'view' }),
  ]);
        ...
        <Card className="mef-animate-in mt-4">
          ...
          {showsCompletedState ? (
            <CompletedExperienceActions
              ...
              retakeAction={retakeLscAction}
            />
          ) : (
            <BeginAssessmentForm action={beginLscAction} label={ctaLabel} className="mt-6" />
          )}
        </Card>
      </main>
      <MemberBottomNav isCoach={isCoach} />
