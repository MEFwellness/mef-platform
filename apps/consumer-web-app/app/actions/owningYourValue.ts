...
/**
 * Owning Your Value's writes, and the coach's read.
 *
 ...
 * scheduled. See lib/owning-your-value/data.ts's header.
 *
 * THE SERVER DECIDES EVERYTHING THE CLIENT COULD HAVE LIED ABOUT. The
 * client posts answers and nothing else. The assignment, the held sentence
...
import {
  findLatestOyvExperiment,
  listOyvDailyLogs,
  upsertOyvDailyLog,
} from '@/lib/owning-your-value/dailyLogsData';
import { clearRootPopupDismissal, owningYourValuePopupMessageKey } from '@/lib/root-popup-messages/data';
import {
  OYV_DEFAULT_DUE_IN_DAYS,
  OYV_DEFINITION_ID,
  OYV_EXPERIENCE_KEY,
} from '@/lib/owning-your-value/constants';
import {
  completeOyvSession,
  fetchOyvSessionForAssignment,
  fetchPendingOyvAssignment,
  listOyvSessions,
  saveOyvDraft,
} from '@/lib/owning-your-value/data';
import {
  OYV_HELD_SENTENCE_KEY,
  OYV_QUESTIONS_VERSION,
  sanitizeOyvAnswers,
  sanitizeOyvDraft,
  type OyvAnswers,
  type OyvDraft,
...
export type SaveOyvDraftResult = { ok: true } | { ok: false; error: string };
 ...
 */
export async function saveOwningYourValueDraftAction(
  draft: unknown
): Promise<SaveOyvDraftResult> {
  const user = await getCachedUser();
  if (!user) return { ok: false, error: 'Please sign in again.' };

  const clean: OyvDraft | null = sanitizeOyvDraft(draft);
  if (!clean) return { ok: false, error: OYV_COPY.submitError };

  const supabase = createClient();
  const assignmentRead = await fetchPendingOyvAssignment(supabase, user.id);
  if (!assignmentRead.ok) return { ok: false, error: OYV_COPY.submitError };
  if (!assignmentRead.assignment) {
    return { ok: false, error: 'Your coach has not opened this one for you.' };
  }

  const record = await saveOyvDraft(supabase, user.id, {
    assignmentId: assignmentRead.assignment.id,
    questionsVersion: OYV_QUESTIONS_VERSION,
    draft: clean,
  });

  // "No error" is not "it worked": saveOyvDraft reads the row back either
  // way, so a write that matched no policy is caught here rather than being
  // reported to her as a save.
  if (!record) return { ok: false, error: OYV_COPY.submitError };
  return { ok: true };
}

export type SubmitOyvResult =
  | { ok: true; sessionId: string; answers: OyvAnswers; heldSentence: string }
 ...
 */
export async function submitOwningYourValueAction(answers: unknown): Promise<SubmitOyvResult> {
  const user = await getCachedUser();
  if (!user) return { ok: false, error: 'Please sign in again.' };

  const clean = sanitizeOyvAnswers(answers);
  if (!clean) return { ok: false, error: OYV_COPY.incompleteError };
  ...
  const existing = await fetchOyvSessionForAssignment(supabase, user.id, assignmentId);
  if (existing?.completedAt && existing.answers) {
    return {
      ok: true,
      sessionId: existing.id,
      answers: existing.answers,
      heldSentence: existing.heldSentence ?? (existing.answers[OYV_HELD_SENTENCE_KEY] ?? ''),
    };
  }

  // sanitizeOyvAnswers has already refused a sheet missing any of the
  // nine, so this key is present. The fallback keeps the compiler honest
  // about an index read rather than asserting.
  const heldSentence = (clean[OYV_HELD_SENTENCE_KEY] ?? '').trim();

  const record = await completeOyvSession(supabase, user.id, {
    assignmentId,
    questionsVersion: OYV_QUESTIONS_VERSION,
    answers: clean,
    heldSentence,
  ...
  if (!record?.completedAt || !record.answers) {
    return { ok: false, error: OYV_COPY.submitError };
  }

  // The pop-up for this assignment can never be due again (the assignment
  // is closed out by migration 211's trigger, so the chain's branch does
  ...
  await clearRootPopupDismissal(supabase, user.id, owningYourValuePopupMessageKey(assignmentId));
  ...
  return {
    ...
    heldSentence: record.heldSentence ?? heldSentence,
  };
}

export type StartOyvExperimentResult = { ok: true } | { ok: false; error: string };

/**
 * Starts the seven day evening experiment she just accepted.
 *
 ...
 */
export async function startOwningYourValueExperimentAction(
  sessionId: string
): Promise<StartOyvExperimentResult> {
  const user = await getCachedUser();
  if (!user) return { ok: false, error: 'Please sign in again.' };
  ...
  const sessions = await listOyvSessions(supabase, user.id);
  ...
  if (activeCount >= MAX_ACTIVE_EXPERIMENTS) {
    return { ok: false, error: OYV_COPY.experimentCapped };
  }
  ...
  const experiment = await startLifestyleExperiment(supabase, user.id, {
    ...
    sourceExperienceKey: OYV_EXPERIENCE_KEY,
    subjectKey: experienceSubjectKey(OYV_EXPERIENCE_KEY),
  });
...
export type CoachOyvPanelState = {
 ...
 * check.
 */
export async function getClientOwningYourValuePanelAction(
  clientId: string
): Promise<CoachOyvPanelState> {
  const empty: CoachOyvPanelState = {
  ...
  const [assignmentRead, sessionRead, timezone] = await Promise.all([
    fetchPendingOyvAssignment(supabase, clientId),
    listOyvSessions(supabase, clientId),
  ...
  return {
    ...
    sessions: sessionRead.records.map((record) => ({
      ...
      answers: record.answers,
      heldSentence: record.heldSentence,
      followUpSourceExperienceKey: record.followUpSourceExperienceKey,
    })),
  };
}

export type AssignOyvResult = { ok: true } | { ok: false; error: string };

/**
 * Assigns Owning Your Value to one client, and does nothing else.
 *
 ...
 * assignStressLoadDeepDiveAction does, so the delivery receipt work shipped
 * on 2026-09-05 has a deadline to measure against.
 *
 ...
 */
export async function assignOwningYourValueAction(
  clientId: string,
  options?: { dueDate?: string }
): Promise<AssignOyvResult> {
  const user = await getCachedUser();
  ...
  const existing = await fetchPendingOyvAssignment(supabase, clientId);
  ...
  const { error } = await supabase.from('assessment_assignments').insert({
    member_id: clientId,
    assessment_definition_id: OYV_DEFINITION_ID,
    assigned_by: user.id,
    is_required: true,
    reason: null,
    stage: 'standard',
...
export type OyvExperimentStatus = {
...
/**
 * Her running Owning Your Value experiment, or null.
 *
 ...
 */
export async function getMyOwningYourValueExperimentAction(): Promise<OyvExperimentStatus | null> {
  const user = await getCachedUser();
  if (!user) return null;

  const supabase = createClient();
  const latest = await findLatestOyvExperiment(supabase, user.id);
  ...
  const logs = await listOyvDailyLogs(supabase, experiment.id);
...
export async function logOwningYourValueDayAction(
  experimentId: string,
  completed: boolean
  ...
  const ok = await upsertOyvDailyLog(supabase, user.id, experimentId, localDate, { completed });
