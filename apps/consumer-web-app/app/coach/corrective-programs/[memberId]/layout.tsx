 ...
 *
 * `/coach/corrective-programs/<memberId>` and the draft review under it
 * each load their own data. Deciding "may this coach see this member at
 * all" on each of them is exactly the per-screen judgement that let the
 * Safety Review Queue drift, so it is decided here instead, once, for
 * every screen in the tree including any added later.
 *
...
export default async function CoachCorrectiveMemberScopeLayout({
  children,
  params,
}: {
  children: ReactNode;
