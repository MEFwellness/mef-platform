...
import { requestAssignSearchQuery, requestDetailSection } from '@/lib/coach-detail/detailBus';
 ...
 * agreeing today.
 *
 * WHAT IT DOES NOT DO. It does not assign anything and it does not replace
 * the Assign panel's own field. Choosing a questionnaire here opens the
 * section, scrolls to that card, and types the query into the field that
 * was always there, which then filters exactly as it does when a coach
 * types into it directly. The send is still the one button on the one
 * panel, so there is still one way to send a thing.
 */
...
export function DetailPageSearch({ assignableTemplates }: Props) {
  ...
  function goToTemplate() {
    setOpen(false);
    // The field gets what she TYPED, not the row's full name. It is the
    // query that filtered the list she just chose from, so the panel opens
    // showing her the same rows she was looking at rather than narrowing
    // to one and hiding the neighbours she was comparing it against.
    requestAssignSearchQuery(query);
    requestDetailSection({ sectionId: ASSIGN_SECTION_ID, anchorId: ASSIGN_CARD_ID });
  }
                    ...
                    {templateResults.map((template) => (
                      <li key={template.id}>
                        ...
                        >
                          ...
                          <span className="mt-0.5 block text-xs text-[#6B7A72]">
                            Opens Assign an Assessment, filtered to this
                          </span>
                        </button>
                      </li>
                    ))}
