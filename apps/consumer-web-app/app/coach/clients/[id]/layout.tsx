/**
 * ONE ANSWER FOR THE WHOLE CLIENT TREE (A3, 2026-08-28).
 *
 * `/coach/clients/<id>` has fifteen screens under it, each loading its own
 * data its own way. Deciding "may this coach see this member at all" on
...
export default async function CoachClientScopeLayout({
  children,
  params,
}: {
  children: ReactNode;
  ...
  if (!(await isMemberVisibleToStaff(supabase, params.id))) notFound();
  return <>{children}</>;
}
