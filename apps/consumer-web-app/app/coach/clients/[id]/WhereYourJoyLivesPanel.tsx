...
/**
 * The coach's Where Your Joy Lives card.
 *
 * BUILT TO FEED THE SESSION, and its order says so.
 *
 * QUESTION SEVEN OPENS IT. "Two hours next week that belonged to nobody but
 * you" is lifted out of the answer list and sits at the top, on its own, in
 * her own words with nothing added, because it is the answer the
 * conversation starts from.
 *
 * IT IS DELIBERATELY NOT THE COLUMN. Question EIGHT is what migration 212
 * stores in twenty_minute_joy, because the small version is the one a later
 * feature could act on. The opener here is the big one. Both are named in
 * lib/where-your-joy-lives/questions.ts so neither is decided by a screen,
 * and this card reads question seven out of the answer sheet it is already
 * rendering rather than asking for a second read.
 *
 ...
 * getClientWhereYourJoyLivesPanelAction, which is where the coach check and
...
import {
  WYJL_ANSWERS_HEADING,
  WYJL_LABEL,
  WYJL_OPENER_HEADING,
  sectionFor,
} from '@/lib/where-your-joy-lives/copy';
import { WYJL_OPENER_KEY, WYJL_QUESTIONS } from '@/lib/where-your-joy-lives/questions';
import {
  assignWhereYourJoyLivesAction,
  type CoachWyjlPanelState,
...
export function WhereYourJoyLivesPanel({
  clientId,
  state,
}: {
  clientId: string;
  state: CoachWyjlPanelState;
}) {
  ...
  function assign() {
    ...
    startTransition(async () => {
      const result = await assignWhereYourJoyLivesAction(clientId);
      if (!result.ok) {
  ...
  return (
    ...
    <section aria-label={WYJL_LABEL} className={`${CARD} p-6`}>
      <div className="flex items-center gap-2 text-[#854D0E]">
        <Sparkles className="h-4 w-4" strokeWidth={1.75} aria-hidden="true" />
        <p className="text-sm font-semibold uppercase tracking-wider">{WYJL_LABEL}</p>
      </div>
      ...
      {state.pendingStatusLine ? (
        <div className="mt-3">
          {/*
            ...
            (getClientWhereYourJoyLivesPanelAction). It says when it was
        ...
        <div className="mt-3">
          ...
          >
            {isPending ? 'Sending' : `Assign ${WYJL_LABEL}`}
          </button>
            ...
            <div className="mt-4 space-y-5">
              ...
              <div className="rounded-2xl border border-[#C4A050]/40 bg-[#FDF9EF] p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap break-words font-[family-name:var(--font-cormorant-garamond)] text-[20px] leading-relaxed text-[#1B3A2D]">
                  {selected.answers[WYJL_OPENER_KEY]}
                </p>
              </div>
                ...
                {([1, 2, 3] as const).map((screen) => (
                  <div key={screen} className="mt-4">
                    ...
                    <dl className="mt-2 space-y-3">
                      {WYJL_QUESTIONS.filter((question) => question.screen === screen).map(
                        (question) => (
