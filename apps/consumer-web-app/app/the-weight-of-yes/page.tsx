 ...
 * getMyTheWeightOfYes the pop-up chain and Home's card read. One rule,
 ...
 * them is made inside TheWeightOfYesExperience, which stays mounted through
 * that re-render.
 *
 * THIS RENDER WRITES NOTHING. No claim, no draft row, no session, and in
 * particular no follow-up flag: the flag is stamped by the first Continue
 * she taps. What this render does is READ which version of question one is
 * due, which is a question, not a decision. See
...
import { TheWeightOfYesExperience } from '@/components/the-weight-of-yes/TheWeightOfYesExperience';

export default async function TheWeightOfYesPage() {
  const user = await getCachedUser();
  if (!user) redirect('/login');

  const state = await getMyTheWeightOfYes();
  ...
  // eight rewrite, and a half-read sheet would render the fixed line over a
  ...
  return (
    <div className={`${CVS_PAGE_BG} font-[family-name:var(--font-dm-sans)]`}>
      <TrackSurfaceView surface="the_weight_of_yes" />
      <main className="mx-auto w-full max-w-md px-5 pb-safe-nav pt-safe-header sm:px-6 md:max-w-2xl md:px-10 md:pb-16 md:pl-28">
        <div className="mt-4">
          <TheWeightOfYesExperience
            status={state.status}
