...
describe('evaluatePair — is the effect-size floor set at a sensible height?', () => {
  // The other fixtures in this file are noiseless (rho exactly ±1 or ~0),
  // which proves the mechanism works but says nothing about where the
  // 0.3 floor actually sits relative to a realistic, scattered
  // relationship. These two are calibrated with a fixed-seed generator
  // (outcome = driver + bounded random noise, clamped back into 1-5) so
  // the underlying rho lands close to a specific target — not asserted
  // against the target, against the real value evaluatePair itself
  // reports, confirmed once by hand before being written in as a fixed
  // fixture (never regenerated at test-run time, so this can never
  // silently drift to a different rho on a future run).
  //
  // Same driver values feed both the "clears the floor" and "refuses the
  // floor" fixtures below at different noise levels, generated by the
  // same LCG-style PRNG used in the heavy-missing-days fixture above,
  // seeds 777 and 888 respectively.

  it('detects a genuinely noisy but real relationship (rho ~0.5) and clears the floor', () => {
    const driver = [2, 2, 3, 3, 4, 3, 5, 5, 3, 2, 1, 2, 4, 3, 3, 3, 1, 2, 4, 3, 3, 3, 4, 4, 2, 2, 2, 2, 3, 3, 3, 3, 3, 1, 4, 1, 3, 2, 3, 2];
    const outcome = [1, 3, 4, 5, 5, 4, 4, 5, 1, 2, 1, 4, 2, 4, 4, 1, 2, 4, 2, 2, 2, 3, 5, 4, 1, 2, 1, 2, 4, 5, 2, 5, 3, 3, 4, 2, 5, 1, 3, 3];

    const result = evaluatePair(seriesFrom('2026-01-01', outcome), seriesFrom('2026-01-01', driver));

    expect(result).not.toBeNull();
    expect(result!.rho).toBeCloseTo(0.5050695154937643, 10);
    expect(Math.abs(result!.rho)).toBeGreaterThan(EFFECT_SIZE_FLOOR);
    expect(result!.direction).toBe('positive');
    expect(result!.lag).toBe('same_day');
    expect(result!.splitWindowAgreement).toBe(true);
  });

  it('refuses a weak relationship (rho ~0.25) that sits under the floor', () => {
    const driver = [2, 5, 5, 4, 4, 4, 5, 4, 4, 3, 3, 4, 5, 3, 3, 3, 4, 2, 4, 2, 3, 2, 4, 3, 2, 3, 4, 5, 3, 2, 5, 3, 1, 2, 5, 5, 4, 2, 4, 3];
    const outcome = [5, 5, 3, 4, 4, 3, 5, 5, 3, 4, 4, 2, 5, 1, 3, 3, 3, 1, 5, 1, 1, 5, 2, 5, 4, 1, 5, 5, 5, 4, 4, 3, 2, 4, 5, 4, 4, 5, 4, 4];

    // The raw same-day rho for this fixture is ~0.267 (confirmed by hand
    // against the same ranking/correlation math evaluatePair itself
    // uses) — below EFFECT_SIZE_FLOOR (0.3). evaluatePair also tries
    // next-day before giving up; this fixture clears neither, so the
    // real, end-to-end assertion is the null it returns, not a rho value
    // that was never allowed to surface.
    const result = evaluatePair(seriesFrom('2026-01-01', outcome), seriesFrom('2026-01-01', driver));
